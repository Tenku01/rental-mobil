...
@section('header', 'Edit Profil Pelanggan')

@section('content')

    <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
            <form action="{{ route('resepsionis.user.update', $user->id) }}" method="POST" class="p-8 space-y-6">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-bold text-gray-500 uppercase">Status Akun</label>
                        <select name="status"
                            class="w-full px-4 py-3 rounded-lg border focus:ring-2 focus:ring-cyan-500 outline-none">
                ...
                <div class="space-y-1">
                    ...
                    <textarea name="alamat" rows="3"
                        class="w-full px-4 py-3 rounded-lg border focus:ring-2 focus:ring-cyan-500 outline-none">{{ old('alamat', $user->pelanggan->alamat ?? '') }}</textarea>
                </div>
